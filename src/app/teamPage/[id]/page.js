...
export default function TeamPage() {
  ...
  // ---- HOOKS (ALL TOGETHER, NO RETURNS ABOVE THIS LINE) ----
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (snap.exists()) {
          setTeamType(snap.data().teamType || "college");
        } else {
          setTeamType("college");
        }
      } catch (err) {
        console.error("Error loading team type:", err);
        setTeamType("college");
      }
    };
    load();
  ...
  // 2) Handle redirect for highschool
  useEffect(() => {
    if (teamType === "highschool" && id) {
      router.replace(`/teamPage/${id}/highschool`);
    }
  }, [teamType, id, router]);

  // 3) Random loading message
  useEffect(() => {
    const randomIndex = Math.floor(Math.random() * hockeySayings.length);
    setMessage(hockeySayings[randomIndex]);
  }, []);

  // 4) Fetch team data, players, games
  useEffect(() => {
    const fetchTeamData = async () => {
      ...
      if (teamSnap.exists()) {
        ...
        if (team.players && team.players.length > 0) {
          ...
          const playersData = (await Promise.all(playerPromises)).filter(
            (p) => p !== null
          );
  ...
  if (teamType === "highschool") {
    // redirect effect already fired; this is just a fallback UI
              ...
              <>
                ...
                <section className="mb-12">
                  ...
                  >
                    ...
                    <div className="mb-10">
                      <p className="text-xl font-extrabold text-black mb-8 tracking-tight border-b border-gray-300 pb-2 mt-6">
                        Forwards
                      </p>
            ...
            {activeTab === "yearOverview" && (
              <div className="bg-white rounded-xl shadow-lg ring-1 ring-gray-200 p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">
                  Here you can display team-wide season stats, charts, and
                  summaries.
                </p>
                <button
  ...
  function renderPlayersByPosition(position) {
    ...
    if (filtered.length === 0) {
      let positionLabel = "";
      if (position === "F") positionLabel = "forwards";
      else if (position === "D") positionLabel = "defense";
      else if (position === "G") positionLabel = "goalies";
      else positionLabel = "players";

      return <p className="text-gray-500">No {positionLabel} found.</p>;
    }
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 p-4">
        {filtered.map((player) => {
          ...
          const fallbackPhoto =
            `/playerPhotos/${player.firstName}${player.lastName}`.toLowerCase() +
            ".jpg";
